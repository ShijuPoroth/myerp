...
async function uploadMaintenancePhotos(logId) {
    ...
    try {
        await fetch(`${API_BASE}/equipment/maintenance-logs/${logId}/photos`, {
            method: 'POST',
            body: formData
        });
    } catch (error) {
...
function printMaintenanceDetails(mode) {
    ...
    const printHtml = `
        ...
        <head>
            ...
            <style>
                @page { margin: 18mm 18mm 22mm 18mm; size: auto; @bottom-right { content: "Page " counter(page) " of " counter(pages); font-size: 10px; color: #999; } }
                * { box-sizing: border-box; }
                body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; color: #1f2937; font-size: 11px; line-height: 1.3; }
                ...
                .doc-header .doc-id .id-label { font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; }
                .doc-header .doc-id .id-value { font-size: 18px; font-weight: 700; color: #1e40af; margin-top: 2px; }
                .section { margin-bottom: 14px; break-inside: avoid; page-break-inside: avoid; }
                .section-title { font-size: 11px; font-weight: 700; color: #374151; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #d1d5db; padding-bottom: 4px; margin-bottom: 8px; }
                .details-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 16px; }
                .detail-row { display: flex; align-items: baseline; }
                .detail-row .label { width: 120px; min-width: 120px; color: #6b7280; font-weight: 500; font-size: 11px; }
                .detail-row .value { flex: 1; font-weight: 600; color: #111827; }
                .description-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 8px; min-height: 40px; white-space: pre-wrap; }
                ...
                table.parts th { padding: 5px 8px; text-align: left; font-weight: 600; font-size: 11px; border: none; }
                table.parts th:last-child { text-align: right; }
                table.parts th:nth-child(3) { text-align: center; }
                table.parts td { padding: 5px 8px; border-bottom: 1px solid #e5e7eb; vertical-align: top; }
                ...
                .signature-label { font-size: 11px; font-weight: 700; color: #374151; margin-bottom: 6px; }
                .signature-name { font-size: 11px; color: #6b7280; margin-bottom: 30px; }
                .signature-line { border-bottom: 1px solid #374151; height: 1px; margin-bottom: 3px; }
                .signature-caption { font-size: 10px; color: #9ca3af; }
                .status-badge { display: inline-block; padding: 2px 8px; border-radius: 999px; font-size: 10px; font-weight: 700; text-transform: uppercase; }
                ...
                .photo-grid img { max-width: 160px; max-height: 120px; border: 1px solid #d1d5db; border-radius: 4px; object-fit: cover; }
...
async function populateMaintenancePartsRows(parts) {
    ...
    // Wait for dropdown options to populate before setting selected values
    await populateMaintenancePartsDropdown();
    
    // Set selected parts after dropdown is populated
    const rows = document.querySelectorAll('.maintenance-part-row');
    rows.forEach((row, index) => {
        if (parts[index]) {
            const hiddenInput = row.querySelector('.maintenance-part-select');
            if (hiddenInput) {
                hiddenInput.value = parts[index].spare_part_id;
                hiddenInput.dataset.cost = parts[index].cost_at_time || 0;
                hiddenInput.dataset.quantity = parts[index].quantity_used || 1;
            }
        }
    });
}
